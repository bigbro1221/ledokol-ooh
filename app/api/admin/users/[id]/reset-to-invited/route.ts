...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Flip back to INVITED, drop any linked OAuth accounts so Google can be
  // re-linked from scratch, then re-issue a fresh activation token.
  await prisma.$transaction([
    prisma.account.deleteMany({ where: { userId: user.id } }),
    prisma.user.update({ where: { id: user.id }, data: { status: 'INVITED' } }),
  ]);
  ...
  try {
    ...
    console.error('[reset-to-invited] email failed', { email: user.email, error: err instanceof Error ? err.message : String(err) });
