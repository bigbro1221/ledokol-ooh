...
export function CampaignTile({ campaign: c, href, locale, dateFormat, statusLabel, screensLabel }: Props) {
  ...
  return (
    ...
    >
      <span className={`absolute right-3 top-3 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.04em] ${STATUS_STYLES[c.status] ?? STATUS_STYLES.DRAFT}`}>
