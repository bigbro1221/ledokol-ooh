...
enum MediaType { SCREENS  OTHER_CARRIERS }

// ScreenType is no longer an enum — it was replaced by the ScreenTypeRef
// reference table (see model definition below). The reference table seeds nine
// codes: LED, STATIC, STOP, AIRPORT, BUS, ROOF, BRANDMAUER, CINEMA, METRO.
// Each Screen carries a typeId FK to ScreenTypeRef.id.
...
model Campaign {
  ...
  totalFinal     BigInt?
  mediaType          MediaType    @default(SCREENS) // SCREENS | OTHER_CARRIERS
  additionalCurrency String?      // free-text currency code paired with additionalAmount
  additionalAmount   BigInt?      // generic secondary-currency total (replaces former totalBudgetRub)
  screens        Screen[]
...
model Screen {
  ...
  externalId        String?
  typeId            String
  screenType        ScreenTypeRef   @relation(fields: [typeId], references: [id])
  city              String
  address           String
  lat               Float?
  lng               Float?
...
// Reference table for screen types — replaces the old ScreenType enum.
// Seeded with 9 rows (LED, STATIC, STOP, AIRPORT, BUS, ROOF, BRANDMAUER,
// CINEMA, METRO). The `category` field (SCREENS | OTHER_CARRIERS) is
// informational only — it does NOT determine which Campaign.mediaType a screen
// belongs to.
model ScreenTypeRef {
  id        String   @id @default(uuid())
  code      String   @unique  // "LED", "ROOF", "BRANDMAUER", ...
  nameRu    String
  nameEn    String
  nameUz    String
  category  String           // "SCREENS" | "OTHER_CARRIERS"
...
### 6.0 Two parsing branches by `Campaign.mediaType`

Every `Campaign` carries a `mediaType` enum (`SCREENS | OTHER_CARRIERS`) that
selects the parser branch and the expected workbook shape:

| `mediaType` | Workbook shape | Entry point | Pricing source |
|-------------|----------------|-------------|----------------|
| `SCREENS` (default) | Multi-sheet (`LED Ташкент`, `Статика`, `Аэропорт`, …) | `lib/parser/index.ts:parseMediaPlan` | Per-row in the file |
| `OTHER_CARRIERS` | Single sheet (`Медиаплан`), one row per `(screen × period)` | `lib/parser/multi-period.ts:parseMultiPeriod` | Entered manually in the campaign form |

For `OTHER_CARRIERS`, period boundaries are derived from column F (`Период
размещения`, formatted `dd.mm.yyyy - dd.mm.yyyy`) by `lib/parser/period.ts`,
and a `CampaignPeriod` row is auto-created on first occurrence of each unique
period string.

The reference template lives at `public/templates/other-carriers-template.xlsx`
and is regenerated by `scripts/build-other-carriers-template.ts`.

**`mediaType` toggle lock.** The mediaType selector on the campaign form is
disabled (and the API rejects changes with `409 mediaType_locked`) once any of
the following exists for the campaign:
- one or more `CampaignPeriod` rows, OR
- any of `totalBudgetUzs`, `productionCost`, `totalFinal`, or `additionalAmount`
  is non-null.

This prevents accidentally orphaning periods or stranding pricing data when
flipping between branches. Server-side enforcement: PUT
`/api/campaigns/[id]`.

### 6.1 Entry Point (SCREENS branch)
