...
async function run() {
  ...
  const byType = await p.screen.groupBy({ by: ['typeId'], _count: true });
  const typeRefs = await p.screenTypeRef.findMany({ select: { id: true, code: true } });
  const codeById = new Map(typeRefs.map((t) => [t.id, t.code]));
  ...
  console.log('\nBy type:');
  byType.forEach((t) => console.log(`  ${codeById.get(t.typeId) ?? t.typeId}: ${t._count}`));
  console.log('\nTop cities:');
